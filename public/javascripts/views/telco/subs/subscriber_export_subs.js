 ...
 */
var tableExportMultiSubs;
function exportMultiSearchSubs(index) {
    console.log(listCheckSubs[index]);
    if(index == 0){
        tableExportMultiSubs = null;
        sttSearch = 0;

        tableExportMultiSubs = document.createElement("table");
        tableExportMultiSubs.setAttribute("class", "table");
        tableExportMultiSubs.setAttribute("id", "myTableExcelMultiSubs");

        document.body.appendChild(tableExportMultiSubs);
        ...
        tableExportMultiSubs.appendChild(tablethread);

        var tablebody=document.createElement("tbody");
        tableExportMultiSubs.appendChild(tablebody);
    }
        ...
        $.ajax({
                    ...
                    if(datas.success){
                        if(datas.content != null){
                            if(datas.content.listSubscriber != null && datas.content.listSubscriber.length > 0){
                                ...
                                $("#myTableExcelMultiSubs").find('tbody').append(tabletr);
                                sttSearch++;
                            }
                        }
                    }
            ...
            complete: function (){
                ...
                if(index < listCheckSubs.length-1){
                    ...
                    outExcelMultiSubs("List_subscriber");
                }
            }
        })
    } else {
