...
function getListProduct(){
    var attrGroupProduct = [];
    $('#listGroupProduct').children().each(function () {
        var checkbox = $(this).find('.groupProduct-check')[0];
        if($(checkbox).is(':checked')){
            attrGroupProduct.push([$(checkbox).val()][0]);
        }
    });

    var body = {
        "filterKey": $("#priceSort").val(),
        "sortKey": "price",
        "groupProduct": (attrGroupProduct.length > 0) ? attrGroupProduct.toString() : "",
