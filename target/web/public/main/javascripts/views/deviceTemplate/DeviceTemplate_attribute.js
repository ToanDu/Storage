...
function getAttributeTemplateList(id) {
    templateId = id;
    var groupData = new FormData;
    groupData.id = id;
    groupData.type = "TEMPLATE";
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.success && data.content != null) {
                viewAtrributeTemplateToTable(data.content.attributes);
            } else {
...
function viewAtrributeTemplateToTable(attrs) {
    console.log(attrs);
    listAttr = new Array;
    ...
    if (attrs!=null && attrs.length > 0) {
        for (var i = 0; i < attrs.length; i++) {
            ...
            var last_update_ts = moment.unix(attrs[i].last_update_ts).format("YYYY/MM/DD HH:mm");
...
function showAddAttribute() {
    ...
    $("#detailAttrConfirm").modal('hide');
}

function processAddAttribute() {
    var formData = new FormData;
    formData.entity_id = templateId;
    formData.attribute_key = $("#nameAttrAdd").val();
    formData.entity_type = "TEMPLATE";
    ...
    $.ajax({
        ...
        success: function (data) {
            console.log(data);
            if(data.success){
                getAttributeTemplateList(templateId);
...
function delAttributeOfOrg() {
    var groupData = new FormData();
    groupData.entity_id = templateId;
    groupData.entity_type= "TEMPLATE";
