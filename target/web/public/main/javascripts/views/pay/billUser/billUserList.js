...
$(document).ready(function () {
    ...
    StartProcess();
    var route = jsRoutes.controllers.PayBillController.getBillOfUser();
    $.ajax({
        type: route.type,
        ...
        success: function (data) {
            console.log(data);
            if (data.success) {
                viewBillUserListToTable(data.content.data);
            } else {
...
function viewBillUserListToTable(data) {
    console.log(data);
    ...
    if (data!=null && data.length > 0) {
        ...
        for (var i = data.length-1; i >= 0; i--) {
            ...
            mtmlAttr += "<td align='center' id='action-"+data[i].id+"'>";
