...
var modelDataListDeviceImport = new Array();
function upload(evt) {
    if (!browserSupportFileUpload()) {
        ...
        reader.onload = function(event) {
            ...
            modelDataListDeviceImport = $.csv.toArrays(csvData);

            if(modelDataListDeviceImport && modelDataListDeviceImport.length > 0){
                console.log(modelDataListDeviceImport);

                if(modelDataListDeviceImport.length > 5000){
                ...
                for(var i=1; i<modelDataListDeviceImport.length; i++){
                    if(modelDataListDeviceImport[i].length == 2){
                        ...
                        var text1 = document.createTextNode(modelDataListDeviceImport[i][0]);
                        td1.appendChild(text1);
                        tabletr.appendChild(td1);

                        var td2=document.createElement("td");
                        var text2 = document.createTextNode(modelDataListDeviceImport[i][1]);
                ...
                $("#titleComfirmImport").text(Messages("qr.importComfirm", modelDataListDeviceImport.length-1));
                $("#divTableImport").show();
            }
        };
