 ...
 */
function importSearchListAccount() {
    console.log("importSearchListAccount");
    document.getElementById("filesSearchAccount").value = "";
    $("#importSearchAccountModal").modal('show');
}

function downloadTemplateSearchAccount() {
    createTableTemplateAccount();
    var filename = "List_Account";
    outExcelAccount(filename);
}
function createTableTemplateAccount() {
    ...
    tablethreadtr.appendChild(createTh(Messages("Số hợp đồng")));

    tablethread.appendChild(tablethreadtr);

    table.appendChild(tablethread);
}
function outExcelAccount(name) {
    console.log(name);
...
//import
var listAccountImport = new Array;
function uploadSearchAccount(evt) {
    listAccountImport = [];
    if (!browserSupportFileUpload()) {
        ...
        if (!file.name.endsWith(".xlsx")) {
            showNotification('danger',Messages("subscriber.import.file.format.error"));
            return;
        }
            ...
            if(listFile.length > 1){
                for(var i=1; i<listFile.length; i++){
                    if(listFile[i][1] != null){
                        if(!listAccountImport.includes(listFile[i][1])){
                            listAccountImport.push(listFile[i][1]);
                        }
                    }
                }
            } else {
                showNotification('danger',Messages("subscriber.import.file.insufficient.data"));
                return;
            }
        };
...
function comfirmListAccountImport() {
    console.log(listAccountImport);
    if(listAccountImport.length > 0){
        var listAccount = "";
        for(var i=0; i<listAccountImport.length; i++){
            if(i == 0){
                listAccount += listAccountImport[i];
            } else {
                listAccount += ","+listAccountImport[i];
            }
        }
        $("#accountId").val(listAccount);
    } else {
        showNotification('danger',Messages("subscriber.import.no.valid.contracts"));
    }
}

//search with list account
function getListByFileAccount(index) {
    console.log(listAccountImport[index]);
    if(index == 0){
    ...
    body.accountId = listAccountImport[index];
    ...
    $.ajax({
        ...
        complete: function (){
            // FinishProcess();
            if(index < listAccountImport.length-1){
                getListByFileAccount(index+1);
            } else {
