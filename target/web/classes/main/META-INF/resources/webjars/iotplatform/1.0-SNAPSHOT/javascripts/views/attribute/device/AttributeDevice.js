...
$(document).ready(function () {
    getAttributeDeviceList();

    searchHistoryAttributeDevice();
});

function getAttributeDeviceList() {
    var groupData = new FormData;
    groupData.id = deviceId;
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.success && data.content != null) {
                viewAtrributeDeviceToTable(data.content.attributes);
            } else {
...
function viewAtrributeDeviceToTable(attrs) {
    console.log(attrs);
    listAttr = new Array;
    ...
    if (attrs!=null && attrs.length > 0) {
        for (var i = 0; i < attrs.length; i++) {
            ...
            if(attrs[i].attribute_type == "SCOPE_SHARE"){
                type = "Share";
            } else if(attrs[i].attribute_type == "SCOPE_CLIENT"){
                type = "Client";
            } else {
                type = "Server";
            }
            ...
            mtmlAttr += "<td style='word-break: break-all; white-space: break-spaces;'>"+value+"</td>";
            ...
            mtmlAttr += '<li style="cursor: pointer; border-bottom: 1px solid #ccc!important;"><a class="dropdown-item" onclick="setEditAttr(\''+attrs[i].attribute_key+'\', \''+attrs[i].attribute_type+'\', \''+attrs[i].value_type+'\', \''+i+'\', \''+attrs[i].logged+'\')"><img src="'+iconEdit+'" alt=""/>&nbsp&nbsp '+Messages("btnEdit")+'</a></li>';
...
function processAddAttribute() {
    ...
    formData.entity_id = deviceId;
    formData.attribute_key = $("#nameAttrAdd").val();
    formData.entity_type = "DEVICE";
...
//edit
var loggerCurrent = "";
function setEditAttr(attribute_key, attribute_type, value_type, i, logger){
    console.log(attribute_key, attribute_type, value_type, i, logger);
    console.log(listAttr[i]);
    $("#nameAttrEdit").val(attribute_key);
    $("#chooseAttrTypeEdit").val(attribute_type);
...
function delAttributeOfOrg() {
    var groupData = new FormData();
    groupData.entity_id = deviceId;
    groupData.entity_type= "DEVICE";
